...
// Run 以 pkg 的形式提供给外部调用
func Run(sql string) (rules []advisor.Rule, out string) {
	common.Config.Query = sql
	out = common.CaptureOutput(func() {
		rules = RunMain()
	})
...
// RunMain 整个 soar cli 的函数迁移到这里
func RunMain() (rules []advisor.Rule) {
	// 全局变量
	var err error
